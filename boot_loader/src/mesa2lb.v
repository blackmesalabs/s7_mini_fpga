...
--                   0x4 = Bus Write Packet ( addr,data,addr,data,.. )
--                   0x5 = Bus Read  Packet ( addr,addr,addr,addr,.. )
...
--  B3  0x04 = Number of Payload Bytes (0-255)
--
-- Note: Bus Read Packet is unfinished and not functional.
--     
--
-- Revision History:
-- Ver#  When      Who      What
...
-- 0.2   10.21.17  khubbard Fix for bytes coming in every other clock
-- 0.2   10.21.17  khubbard Added 0x4 Write Packet command             
-- ***************************************************************************/
...
// 2017.10.21
 if ( rx_byte_rdy_p1 == 1 && byte_cnt[1:0] == 2'd3 ) begin
   dword_rdy <= 1;
   header_jk <= 0;
   if ( header_jk == 1 && wr_jk == 0 && rd_jk == 0 ) begin 
     ...
     if ( dword_sr[31:16] == 16'hF000 ) begin
       ...
       if ( dword_sr[15:12] == 4'h0 || dword_sr[15:12] == 4'hE ) begin
         ...
         if ( dword_sr[11:8] == 4'h4 ) begin
           wr_pkt_jk   <= 1;
           dword_stage <= 3'b001;
         end 
//       if ( dword_sr[11:8] == 4'h5 ) begin
//         rd_pkt_jk   <= 1;
//         dword_stage <= 3'b001;
//       end 
     ...
     if ( wr_pkt_jk == 1 ) begin
       dword_stage[0] <= ~ dword_stage[0];
     end
//   if ( rd_pkt_jk == 1 ) begin
//     dword_stage[1] <=  0;
...
always @ ( posedge clk ) begin : proc_lb2
  ...
  // Note: wr_pkt_jk_p1 holds off lb_wr until both addr and data are available
  if ( dword_rdy == 1 && wr_pkt_jk_p1 == 1 ) begin
    if ( dword_stage[0] == 0 ) begin
...
//    addr_len      <= 32'd1;         // HACK FIXED to single DWORD BREAKS SUMP
      ro_header_rdy <= 1;// Send Ro Header as we know length of packet to send
//    if ( mode_usb3 == 1 ) begin 
//      addr_len <= 32'd1; // HACK FIXED to single DWORD BREAKS SUMP
//    end 
    end
  end // if ( dword_rdy == 1 && rd_jk == 1 ) begin

//if ( dword_rdy == 1 && rd_pkt_jk == 1 ) begin
//  if ( dword_stage[1] == 1 ) begin
//    addr_len      <= dword_sr[31:0];// 1st DWORD of RD is Num DWORDs to Read
//    ro_header_rdy <= 1;// Send Ro Header as we know length of packet to send
//  end
//  if ( dword_stage[2] == 1 ) begin
//    addr_cnt      <= dword_sr[31:0];// 2nd DWORD of RD is LB Address
//  end
//end // if ( dword_rdy == 1 && rd_jk == 1 ) begin
...
//rd_timeout_cnt_p1 <= rd_timeout_cnt[7];
  rd_timeout_cnt_p1 <= rd_timeout_cnt[9];

  // Timeout after 128 clocks
  if ( reset == 1 ) begin
    rd_timeout_cnt <= 10'H000;// Stopped
  end else begin
    if ( lb_user_jk == 1 || lb_prom_jk == 1 ) begin
      if ( lb_rd_loc == 1 ) begin
        ...
        rd_timeout_cnt[9] <= 0;// Stop the Count
        rd_timeout_cnt_p1 <= 0;
      end else if ( rd_timeout_cnt[9] == 1 ) begin
